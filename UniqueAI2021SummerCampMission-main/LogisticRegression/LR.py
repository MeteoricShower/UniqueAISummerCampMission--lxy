...
def sigmoid(x):
    ...
    for i in range(w):
        for j in range(h):
            if x[i][j] >= 0:
                y.append(1.0 / (1 + np.exp(-x[i][j])))
            else:
                y.append(np.exp(x[i][j]) / (np.exp(x[i][j]) + 1))
...
class LogisticRegression(object):
    ...
    def __init__(self, penalty='l2', tol=0.0001, C=1.0, bias=True, max_iter=10000,learning_rate=0.0001):
        '''
    ...
    def fit_transform(self,x_train,y_train):
        ...
        self.gradient_function(x_train,y_train)
    ...
    def score(self,x_test,y_test):
        ...
        for i in range(numSamples):
            predict = sigmoid(x_test[i, :] * self.theta[1::])[0, 0] > 0.5
            if predict == bool(y_test[i]):
                matchCount += 1
        score = float(matchCount) / numSamples
        return score
    ...
    def gradient_function(self,data,label):
        ...
        dataMatrix = np.insert(np.mat(data), 0, 1, axis=1)
        labelMat = np.mat(label).transpose()
        m, n = np.shape(dataMatrix)
        self.theta = np.ones((n, 1))
        for i in range(self.max_iter):
            output = sigmoid(dataMatrix * self.theta)
            error = labelMat - output
            self.theta = self.theta + self.learning_rate * (dataMatrix.transpose() * error+self.C*self.theta)
        return self.theta
